(function ($, window, document, undefined) {
  ...
  Design_Polygon.prototype={
    ...
    mapSetup:function(){
      ...
      poly_obj.map = map = new google.maps.Map(
      ...
      if(style!=''){
        map.setOptions({styles:eval(style)});
      }
    },
    ...
    drawSavedPolygon:function(){
      var poly_obj = this;
      onepolygondraw=this.savedpolygon;
      ...
      for(var i=0,l=onepolygondraw.length;i<l;i++) {
          google.maps.event.addListener(onepolygondraw[i], 'click', function(event) {
            ...
            for (var i = 0; i < this.getPath().getLength(); i++) {
              ...
              var singlevertex = { lat:lat, lng:lng };
              showpolycoordinate.push(singlevertex);
            }

            $(".wpdap_fill_color").iris('color', this.fillColor);
            $('.wpdap_stroke_color').iris('color', this.strokeColor);
            $('#wdap_shape_stroke_weight').select2().select2('val',this.strokeWeight);
            $('#wdap_shape_stroke_opacity').select2().select2('val',this.strokeOpacity);
            $('#wdap_shape_fill_opacity').select2().select2('val',this.fillOpacity);
      ...
      google.maps.event.addListener(poly_obj.drawingManager, 'overlaycomplete', function (e) {
        ...
        if (e.type !== google.maps.drawing.OverlayType.MARKER) {
            ...
            google.maps.event.addListener(newShape, 'click', function (e) {
              ...
              $('#wdap_shape_stroke_weight').select2().select2('val',$('#'+this.id).attr('data-strokeweight'));
              $('#wdap_shape_stroke_opacity').select2().select2('val',$('#'+this.id).attr('data-strokeopacity'));
              $('#wdap_shape_fill_opacity').select2().select2('val',$('#'+this.id).attr('data-fillopacity'));
    ...
    $('#wdap_shape_path').change(function(e){
       if (poly_obj.selectedShape) {
         ...
         poly_obj.checkchangeattribute(this.selectedShape,'coordinatepath',updatedshap);
       }
    });
      ...
      $('#addcollection').submit(function(event){
         var jk=[];
         for ( i in poly_obj.completepolygonscordinates) {
           var polyid= poly_obj.completepolygonscordinates[i][0].id;
           var formt=poly_obj.completepolygonscordinates[i][0].popygon_all_properties;
           var newformating={redirectUrl:'',infoWindow:''};
            ...
            poly_obj.completepolygonscordinates[i][0].popygon_all_properties = newformating;
          }
