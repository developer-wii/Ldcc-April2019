 ...
 */
//WDAP_TEXT_DOMAIN
if ( class_exists( 'WP_List_Table_Helper' ) and ! class_exists( 'WDAP_Backup_Table' ) ) {

	/**
	 * Display backup manager.
	 */
	class WDAP_Backup_Table extends WP_List_Table_Helper {
		/**
		...
		function column_backup_file_name($item) {
			...
			$file_path = WDAP_BACKUP_URL.$item->backup_file_name;
	...
	$form  = new WDAP_FORM();
	$form->set_header( __( 'Take Collection Backup', WDAP_TEXT_DOMAIN ), $respone_take_backup );
	...
	$form->add_element('message','backup_message',array(
		'value' => __( 'Click below to create a backup of all collections.',WDAP_TEXT_DOMAIN ),
		'class' => 'alert alert-info',
	));

	$form->add_element('submit', 'wdap_save_backup', array(
		'value' => __( 'Create Backup',WDAP_TEXT_DOMAIN ),
	));

	$form->render();

	$import_form = new WDAP_FORM(array('no_header' => true ));
	$import_form->set_header( __( 'Upload Backup', WDAP_TEXT_DOMAIN ), $respone_upload_backup );
	$import_form->add_element('hidden', 'operation', array(
		'value' => 'upload_backup',
	));

	$import_form->add_element('file','uploaded_file',array(
		'label' => __( 'Choose File',WDAP_TEXT_DOMAIN ),
		'desc' => __( 'Please upload a valid collection backup sql file.',WDAP_TEXT_DOMAIN ),
		'class' => 'file_input'
	));
	$import_form->add_element('submit', 'wdap_backup_submit', array(
		'value' => __( 'Upload Backup',WDAP_TEXT_DOMAIN ),
	));
	$import_form->render();
