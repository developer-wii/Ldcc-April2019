...
if (!defined('XC_WOO_CLOUD_VERSION'))
    define('XC_WOO_CLOUD_VERSION', '2.1');
    ...
    final class XC_WOO_CLOUD
    {
        ...
        */
        public function __construct()
        {
			...
			add_action( 'wp_ajax_xc_woo_cloud_dismiss_admin_notice', array( &$this, 'xc_dismiss_admin_notice' ) );
			...
			add_action('woocommerce_checkout_order_processed', array(&$this, 'xc_order_processed'),10,3);
			add_action('woocommerce_payment_complete', array($this, 'xc_order_payment_complete'));
        ...
        */
        public function init()
        {
            ...
            if (is_admin()):
				...
				$review_request = get_option("xc_woo_cloud_review_request");
				
				if(empty($review_request)){
					$today = date("Y-m-d");
					$time = strtotime($today. ' + 3 days');
					$opt = array("option"=>"later","time"=>$time);
					update_option("xc_woo_cloud_review_request",$opt);				 	
				}else{				
					if((isset($review_request['time']) && $review_request['time'] < time())){
		...
		public function review_request(){
			?>
			<div class="updated xc-review-request" id="xc-review-request">
            	<h3>Dear customer!</h3>
                	<p>Thank you for purchasing and activating « <strong>Woocommerce Google Cloud Print</strong> » Plugin!</p>
                   <p>If you liked our plugin, please <a target="_blank" href="http://codecanyon.net/downloads">rate it with 5 stars</a>, it will help us to make regular updates and support this plugin as long as it will be in demand.</p>
<p>If you are not satisfied with our product, please do not rush to put a bad rating, contact us about your wishes. We can not guarantee that all of them will be implemented, but the most interesting and popular ideas will be added necessarily!</p>
<p>Plugin rating is related to further plugin development and support directly, please encourage our work!</p>
<p>Thank you, and Good luck. Best regards, <strong>Xperts Club</strong> team.</p>
                <p class="xc-admin-buttons">
                <strong><a href="javascript:void(0)" class="xc-notice-dismiss" data-id="dismiss">Dismiss this notice </a> &nbsp;&nbsp; | &nbsp;&nbsp <a href="javascript:void(0)" class="xc-notice-dismiss" data-id="later">Remind me later</a></strong>
                </p>
                </div>
                <script type="text/javascript">
				jQuery(document).on("click",".xc-notice-dismiss",function(){
					jQuery("#xc-review-request").slideUp();	
					var $d = jQuery(this).data("id");
					jQuery.post(ajaxurl,{"action":"xc_woo_cloud_dismiss_admin_notice","type":$d},function(data){
					})
				});
				</script>	
            <?php
		...
		function xc_dismiss_admin_notice(){
			$type = $_POST['type'];
			$opt = array("option"=>"dismiss");
			if($type != "dismiss"){
				$today = date("Y-m-d");
				...
				$option = get_option("xc_woo_cloud_review_request");
				if(!empty($option) && $option['option']!="dismiss"){				 	
					$today = date("Y-m-d");
					$time = strtotime($today. ' + 25 days');
					$opt = array("option"=>"later","time"=>$time);
					update_option("xc_woo_cloud_review_request",$opt);				 	
				}else{
					update_option("xc_woo_cloud_review_request",$opt);
        ...
        function xc_print($data, $printjobtitle, $printer)
        {
			...
			//$ticket['print']['media_size'] = array("width_microns"=>"74000","height_microns"=>"105000","is_continuous_feed"=>false);
			...
			if(!empty($settings["{$document_type}_printers"])){
				foreach($settings["{$document_type}_printers"] as $k=>$v){
					if(isset($v['enable']) && $v['enable'] == '1'){
						...
						$ret = $this->xc_print($data, $document_title,$printer);
						echo ($ret);
					}
				}
			}
		...
		function xc_woo_printer_preview(){
			...
			if($return){ 
				...
				$document_title = apply_filters('xc_woo_cloud_print_document_title',$document_title,$order_id,$document_type);
				$data = file_get_contents($file);
				$this->xc_print($data, $document_title,$printer);
		...
		private function xc_woo_cron_print($order_id) {
			$time = apply_filters("xc_woo_cloud_print_cron_delay",current_time( 'timestamp' ));
