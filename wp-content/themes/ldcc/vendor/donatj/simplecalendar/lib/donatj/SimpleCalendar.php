...
class SimpleCalendar {
	...
	public function setDate( $date_string = null ) {
        ...
        public function setTableId($ids = null)
        {
            if($ids == null)
            {
                $this->tableId = 'SimpleCalendar';
            }
            else
            {
                $this->tableId = $ids;
        ...
        public function assignAlreadyChosenDates($currDates)
        {
            if(!empty($currDates))
            {
                return $this->already_chosen_date = $currDates;
            }
            return null;
        }
	...
	public function show( $echo = true ) {
		...
		$no_days = cal_days_in_month(CAL_GREGORIAN, $this->now['mon'], $this->now['year']);
                ...
                {
                    $out = '<table cellpadding="0" cellspacing="0" class="SimpleCalendar" id="'.$this->tableId.'"><thead><tr>';
                }
                else
                {
		...
		for( $i = 1; $i <= $no_days; $i++ ) {
			...
			$datetime = mktime(0, 0, 1, $this->now['mon'], $i, $this->now['year']);
                        ...
                        if($this->already_chosen_date == date('Y-m-d', $datetime))
                        {
			$out .= '<time class="date-cell modal-date-selector saved-date" datetime="' . date('Y-m-d', $datetime) . '" onClick="dateSelector(this);">' . $i . '</time>';  
                        }
                        else {
			$out .= '<time class="date-cell modal-date-selector" datetime="' . date('Y-m-d', $datetime) . '" onClick="dateSelector(this);">' . $i . '</time>';
                        }
